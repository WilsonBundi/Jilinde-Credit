// Demo Service for Frontend-Only Deployment (No Backend Required)
...
export const demoService = {
  ...
  approveApplication: async (applicationId) => {
    ...
    if (customer) {
      ...
      mockStats.pendingApplications = Math.max(0, mockStats.pendingApplications - 1);
      mockStats.approvedApplications += 1;
    }
    return {
      data: {
        status: 'approved',
  ...
  rejectApplication: async (applicationId, reason) => {
    ...
    if (customer) {
      ...
      mockStats.pendingApplications = Math.max(0, mockStats.pendingApplications - 1);
      mockStats.rejectedApplications += 1;
    }
    return {
      data: {
        status: 'rejected',
...
export const isDemoMode = () => {
  // If REACT_APP_API_URL is explicitly set, use real backend
  if (process.env.REACT_APP_API_URL) {
    return false;
  }
  
  // If on Railway, Heroku, or similar platforms with backend, use real backend
  ...
  // Demo mode for Netlify, GitHub Pages, or local development without backend
